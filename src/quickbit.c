...
quickbit_get (const quickbit_t field, size_t len, int64_t bit) {
  ...
  return quickbit__get_unchecked(field, (uint64_t) bit);
}
...
quickbit_set (quickbit_t field, size_t len, int64_t bit, bool value) {
  ...
  return quickbit__set_unchecked(field, (uint64_t) bit, value);
}
...
quickbit_find_first (const quickbit_t field, size_t len, bool value, int64_t position) {
  ...
  while ((i & 127) != 0 && i < n) {
    if (quickbit__get_unchecked(field, i) == value) return i;
    i++;
  }
...
quickbit_index_init (quickbit_index_t index, const quickbit_t field, size_t len) {
  for (int64_t i = 0; i < 128; i++) {
    for (int64_t j = 0; j < 128; j++) {
      ...
      quickbit__set_unchecked(index, quickbit_index__bit_offset(0, k), allz);

      quickbit__set_unchecked(index, quickbit_index__bit_offset(1, k), allo);
    }
...
quickbit_index__update_propagate (quickbit_index_t index, int64_t bit, simdle_v128_t vec) {
  ...
  if (quickbit__set_unchecked(index, quickbit_index__bit_offset(0, 128 + j), simdle_allz_v128(vec))) {
    changed = true;

    bool allo = simdle_allo_v128(simdle_load_v128_u8(&index[quickbit_index__byte_offset(0, i * 16 + 16)]));

    quickbit__set_unchecked(index, quickbit_index__bit_offset(0, i), allo);
  }

  if (quickbit__set_unchecked(index, quickbit_index__bit_offset(1, 128 + j), simdle_allo_v128(vec))) {
    changed = true;

    bool allo = simdle_allo_v128(simdle_load_v128_u8(&index[quickbit_index__byte_offset(1, i * 16 + 16)]));
...
quickbit_index_update (quickbit_index_t index, const quickbit_t field, size_t len, int64_t bit) {
  ...
  return quickbit_index__update_propagate(index, bit, simdle_load_v128_u8(&field[bit / 128 * 16]));
}
...
quickbit_index_update_sparse (quickbit_index_t index, const quickbit_chunk_t chunks[], size_t len, int64_t bit) {
  ...
  return quickbit_index__update_propagate(index, bit, simdle_load_v128_u8(&chunk->field[offset - chunk->offset]));
}
...
quickbit_index_fill (quickbit_index_t index, const quickbit_t field, size_t len, bool value, int64_t start, int64_t end) {
  ...
  if ((start & 127) != 0) {
    simdle_v128_t vec = simdle_load_v128_u8(&field[j * 16]);

    quickbit__set_unchecked(index, quickbit_index__bit_offset(0, 128 + j), simdle_allz_v128(vec));

    quickbit__set_unchecked(index, quickbit_index__bit_offset(1, 128 + j), simdle_allo_v128(vec));

    j++;
  }

  if ((end & 127) != 0 && j < m) {
    simdle_v128_t vec = simdle_load_v128_u8(&field[(m - 1) * 16]);

    quickbit__set_unchecked(index, quickbit_index__bit_offset(0, 128 + m - 1), simdle_allz_v128(vec));

    quickbit__set_unchecked(index, quickbit_index__bit_offset(1, 128 + m - 1), simdle_allo_v128(vec));
...
quickbit_skip_first (quickbit_index_t index, size_t len, bool value, int64_t position) {
  ...
  while (i <= 127 && quickbit__get_unchecked(index, quickbit_index__bit_offset(value, i))) {
    i++;
  }
  ...
  while (j <= 127 && quickbit__get_unchecked(index, quickbit_index__bit_offset(value, i * 128 + j + 128))) {
    j++;
    k += 128;
  }
...
quickbit_skip_last (quickbit_index_t index, size_t len, bool value, int64_t position) {
  ...
  while (i >= 0 && quickbit__get_unchecked(index, quickbit_index__bit_offset(value, i))) {
    i--;
  }
  ...
  while (j >= 0 && quickbit__get_unchecked(index, quickbit_index__bit_offset(value, i * 128 + j + 128))) {
    j--;
    k -= 128;
  }
